...
START_COLLECTHSMETRICS_MT=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD=${CMD}" CollectHsMetrics"
			CMD=${CMD}" --INPUT ${CORE_PATH}/${PROJECT}/TEMP/${SM_TAG}/${SM_TAG}_MT.bam"
		...
		CMD=${CMD}" --PER_BASE_COVERAGE ${CORE_PATH}/${PROJECT}/MT_OUTPUT/COLLECTHSMETRICS_MT/${SM_TAG}_per_base_cov.tsv"
		CMD=${CMD}" --OUTPUT ${CORE_PATH}/${PROJECT}/MT_OUTPUT/COLLECTHSMETRICS_MT/${SM_TAG}.output.metrics"
...
# write out timing metrics to file
	...
	echo ${SM_TAG}_${PROJECT},B01,COLLECTHSMETRICS_MT,${HOSTNAME},${START_COLLECTHSMETRICS_MT},${END_COLLECTHSMETRICS_MT} \
