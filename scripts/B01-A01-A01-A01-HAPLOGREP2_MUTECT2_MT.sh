...
START_HAPLOGREP2_MUTECT2_MT=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD=$CMD" /jars/haplogrep-2.1.20.jar"
			CMD=$CMD" --in ${CORE_PATH}/${PROJECT}/MT_OUTPUT/MUTECT2_MT/${SM_TAG}.MUTECT2_MT_FILTERED_MASKED.vcf"
			CMD=$CMD" --extend-report"
			CMD=$CMD" --format vcf"
			CMD=$CMD" --hits 5"
		CMD=$CMD" --out ${CORE_PATH}/${PROJECT}/MT_OUTPUT/HAPLOGROUPS/${SM_TAG}.haplogroups.txt"
...
# write out timing metrics to file
	...
	echo ${SM_TAG}_${PROJECT},E01,HAPLOGREP2_MUTECT2_MT,${HOSTNAME},${START_HAPLOGREP2_MUTECT2_MT},${END_HAPLOGREP2_MUTECT2_MT} \
