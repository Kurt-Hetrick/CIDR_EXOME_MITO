...
START_COVERAGE_MT=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD="singularity exec ${MITO_MUTECT2_CONTAINER} Rscript"
			...
			# input file
			CMD=${CMD}" ${CORE_PATH}/${PROJECT}/MT_OUTPUT/COLLECTHSMETRICS_MT/${SM_TAG}_per_base_cov.tsv"
		...
		CMD=${CMD}" ${CORE_PATH}/${PROJECT}/MT_OUTPUT/COLLECTHSMETRICS_MT"
