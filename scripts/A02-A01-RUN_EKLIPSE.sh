...
START_RUN_EKLIPSE=`date '+%s'` # capture time process starts for wall clock tracking purposes.
	...
	# construct command line
		...
		CMD="singularity exec ${MITO_EKLIPSE_CONTAINER} eKLIPse.py"
			CMD=${CMD}" -in ${CORE_PATH}/${PROJECT}/TEMP/${SM_TAG}_MT/${SM_TAG}_EKLIPSE_CONFIG.txt"
			CMD=${CMD}" -ref ${MT_GENBANK}"
			CMD=${CMD}" -thread ${THREADS}"
			CMD=${CMD}" -downcov 0"
			CMD=${CMD}" -scsize 15"
			CMD=${CMD}" -mapsize 10"
		CMD=${CMD}" -out ${CORE_PATH}/${PROJECT}/TEMP/${SM_TAG}_MT/EKLIPSE"
...
# write out timing metrics to file
	...
	echo ${SM_TAG}_${PROJECT},C01,RUN_EKLIPSE,${HOSTNAME},${START_RUN_EKLIPSE},${END_RUN_EKLIPSE} \
